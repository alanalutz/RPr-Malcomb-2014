...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Alana Lutz, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
The study's social impacts are to address extreme vulnerability to climate change (1.3) and assist in the allocation and evaluation of foreign aid (1.2).
...
> H2: There is no perfect positive correlation between Malcomb et al's ranking of locations by climate vulnerability and our reproduction study's ranking of locations by climate vulnerability.
...
- Vulnerability = Assets + Access + Livelihood sensitivity + (40 - Physical Exposure) 9.
...
# changed from tmap to ggplot2
tmap_mode('plot')
tm_shape(lhz_v) +
  tm_polygons("sensitivity", palette = c("lemonchiffon1", "lightcoral"), style="cont", title = "Livelihood\nSensitivity\nScores", border.col="white") +
  tm_layout(legend.outside = TRUE, frame=FALSE) +
  tm_grid(labels.col="gray", lines=FALSE)
...
To modify this analysis, I will also calculate an aggregated vulnerability score using a geometric aggregation rather than an additive aggregation.
I added a small value (1) to each indicator to remove 0 values, then multiplied the indicators together, subtracted 1, and calculated the square root.
Finally, I log transformed the data for ease of visualization and comparison.

$$
Vulnerability = log(sqrt((40 - Adaptive Capacity) * Livelihood Sensitivity * Drought Exposure * Flood Risk))
$$

```{r combined-vulnerability-geometric}
# modify data to remove 0 values
capacity_g <- capacity_r + 1
sensitivity_g <- sensitivity_r + 1
drought_g <- drought_r + 1
flood_g <- flood_r + 1
# final output (adding component rasters)
# log transforming the data makes the map easier to interpret
vulnerability_g <- log(sqrt((40 - capacity_g) * sensitivity_g * drought_g * flood_g) - 1)
names(vulnerability_g) <- "vulnerability"
plot(vulnerability_g)
```

```{r find break & limit values for vulnerability maps}
vuln_max <- max(vulnerability_r[[1]],na.rm = TRUE)
vuln_min <- min(vulnerability_r[[1]],na.rm = TRUE)

vuln_g_max <- max(vulnerability_g[[1]],na.rm = TRUE)
vuln_g_min <- min(vulnerability_g[[1]],na.rm = TRUE)
```

### Reproduction figure 5

Reproduce figure 5 using the additive aggregation method from the original paper.

```{r vulnerability-tmap}
tmap_mode('plot')
tm_shape(vulnerability_r) +
  tm_raster(palette = c("#FFFF75", "#CF4611"), style="pretty", breaks = c(vuln_min, vuln_max),
            labels = c("Lower Vulnerability", "", "", "", "", "Higher Vulnerability"), title = "", legend.hist=TRUE) +
  tm_layout(legend.outside = TRUE, main.title = "Malawi Vulnerability to Climate Change:\nAdditive Aggregation", main.title.size = 1, frame=FALSE) +
  tm_grid(labels.col="gray", lines=FALSE)

```

\
Reproduce figure 5 using the geometric aggregation method, a deviation from the original paper for the purpose of reanalysis.

```{r vulnerability-geometric-tmap}
tmap_mode('plot')
tm_shape(vulnerability_g) +
  tm_raster(palette = c("#FFFF75", "#CF4611"), style="pretty", breaks = c(vuln_g_min, vuln_g_max),
            labels = c("Lower Vulnerability", "", "", "", "Higher Vulnerability"), title = "", legend.hist=TRUE) +
  tm_layout(legend.outside = TRUE, main.title = "Malawi Vulnerability to Climate Change:\nGeometric Aggregation (Log Transformed)", main.title.size = 1, frame=FALSE) +
  tm_grid(labels.col="gray", lines=FALSE)
...
# calculate difference between additive and geometric aggregations
rpvmin_g <- min(vulnerability_g$vulnerability, na.rm= TRUE)
rpvmax_g <- max(vulnerability_g$vulnerability, na.rm= TRUE)
vulnerability_g = vulnerability_g %>% 
  mutate(rpv = (vulnerability - rpvmin_g) / (rpvmax_g - rpvmin_g) * 100)
 # rpv is Re-scaled from 0 to 100 with (value - min)/(max - min)
rm(rpvmin_g, rpvmax_g)

vulnerability_g$diffv <- vulnerability_g$rpv - vulnerability_r$rpv
...
Map differences in Figure 5 between the reproduction and the original study.
...
tmap_mode('plot')
tm_shape(vulnerability_r) +
  tm_raster(col = "diffv", palette = "PiYG", style = "cont", breaks = c(-range,0,range), title = "Vulnerability:\nReproduction - Original") +
  tm_layout(legend.outside = TRUE, main.title.size = 1, frame=FALSE) +
  tm_grid(labels.col="gray", lines=FALSE)
```

Map differences in Figure 5 between the reproduced additive and geometric aggregation methods.

```{r vulnerability-difference-map2}
# find range of values for divergent color scheme
range <- ceiling(max(abs(max(vulnerability_g$diffv, na.rm = TRUE)),
             abs(min(vulnerability_g$diffv, na.rm = TRUE))))
...
The intent of the original study published by Malcomb et al. was to maximize replicability across space, time, and vulnerability stimuli, addressing critiques of the uncertainty and sensitivity of vulnerability models.
However, despite the authors' intentions to publish a fully reproducible study, implementing an actual reproduction in practice raises a host of challenges that highlight more general problems with reproducibility in the field of spatial analysis.
This reproduction attempt ultimately failed to produce identical or even similar results to the original study, as a result of a number of causes.

For example, it was necessary to make many assumptions about calculating the individual components of adaptive capacity indicators from household survey data.
The original study was unclear about the process for converting data (especially binary data) to a 5-point quintile scale, and the reproduction instead scaled each indicator from 0 to 4 using percent rank.
Before even examining vulnerability data, there were digitization errors involving lakes, conservation areas, and the treatment of geometry errors and missing data that needed to be resolved before the map of populated Traditional Authorities could be considered accurate.
In addition, in order to compare the results of the reproduction to that of the original study, the figures from the original study needed to be digitized using only the image from the pdf.
This process seems unnecessarily complicated and could have been eliminated by publishing the figures in an editable format, or at least by clarifying the exact methods used to produce those figures.
Beyond the technical and computational aspects of reproduction, the communication of the original study hindered overall transparency and clarity by blackboxing their stakeholder interview process and failing to provide consistent definitions of "vulnerability," "resilience," or "adaptive capacity."

When taking all of these reproduction challenges into consideration, it is not surprising that there were major differences between the original and reproduced adaptive capacity results (Figure 4) across Malawi, as evidenced by the confusion matrix.
There were also major differences between the original and reproduced vulnerability results (Figure 5).
In particular, the reproduction indicated much lower vulnerability scores in central Malawi.
Overall, this reproduction can be considered a failure, if success is measured by the ability to produce identical or very similar results to the original study.
...
This attempt at reproducing the climate vulnerability analysis in Malcomb et al. ultimately failed to produce identical or even similar results to the original study, despite the original study's intentions of reproducibility.
