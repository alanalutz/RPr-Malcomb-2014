...
vuln_map = ggplot() +
  ...
  labs(title = "Malawi Vulnerability to Climate Change: Additive Aggregation") +
  theme_minimal() +
  theme(
...
tm_shape(vulnerability_r) +
  ...
  tm_layout(legend.outside = TRUE, main.title = "Malawi Vulnerability to Climate Change: Additive", main.title.size = 1)

```


```{r vulnerability-map-geometric}
vuln_map = ggplot() +
  geom_stars(data = vulnerability_g) +
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(vuln_g_min, vuln_g_max),
    labels = c("Lower Vulnerability", "Higher Vulnerability"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(vuln_g_min, vuln_g_max)
  ) +
  labs(title = "Malawi Vulnerability to Climate Change: Geometric Aggregation") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
